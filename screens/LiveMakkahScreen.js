...
export default function LiveMakkahScreen() {
  ...
  // Function to find current live stream from Makkah
  const findLiveStream = async () => {
    ...
    try {
      ...
      if (allVideos.length === 0) {
        throw new Error('لا يتوفر بث مباشر من الحرم المكي حالياً');
      }

      // تصفية الفيديوهات للحصول على المرتبطة بالحرم المكي
      const makkahVideos = allVideos.filter(item => {
        const title = item.snippet.title.toLowerCase();
        const description = item.snippet.description.toLowerCase();
        const channelTitle = item.snippet.channelTitle.toLowerCase();
        
        // كلمات مفتاحية للبحث عن فيديوهات الحرم المكي
        const keywords = [
          'مكة', 'حرم', 'كعبة', 'makkah', 'kaaba', 'masjid al haram', 
          'holy mosque', 'grand mosque', 'الحرم المكي', 'المسجد الحرام',
          'haramain', 'saudi', 'السعودية'
        ];
      ...
      const targetVideos = makkahVideos.length > 0 ? makkahVideos : allVideos;
      ...
      const sortedVideos = targetVideos.sort((a, b) => {
        ...
        const priorityKeywords = ['مكة', 'حرم', 'كعبة', 'makkah', 'kaaba'];
  ...
  const injectedJavaScript = `
    (function() {
      ...
      // محاولة تشغيل الصوت وجعل الفيديو fullscreen
      ...
      <View style={[userOrientation === 'portrait' ? styles.containerPortrait : styles.container, styles.centerContent]}>
        {userOrientation === 'portrait' && (
          <View style={styles.headerPortrait}>
            ...
            <Text style={styles.titlePortrait}>البث المباشر من الحرم</Text>
          </View>
        )}
    ...
    <View style={userOrientation === 'portrait' ? styles.containerPortrait : styles.container}>
      ...
      <View style={userOrientation === 'portrait' ? styles.videoContainerPortrait : styles.videoContainer}>
        {currentVideoId && (
          <WebView
            ref={webViewRef}
            source={{ 
              uri: `https://www.youtube.com/embed/${currentVideoId}?autoplay=1&mute=0&enablejsapi=1&playsinline=0&loop=0&rel=0&modestbranding=1&controls=1`
            }}
            ...
            onLoadStart={() => {
              // تحسين الأداء - إيقاف مؤشر التحميل سريعاً
            }}
            onLoadEnd={() => {
              // تشغيل الفيديو فوراً بعد التحميل
              setTimeout(() => {
                webViewRef.current?.injectJavaScript(injectedJavaScript);
              }, 500);
            }}
      ...
      {/* {userOrientation === 'portrait' && (
        <View style={styles.infoContainerPortrait}>
          <Text style={styles.infoTextPortrait}>البث المباشر من الحرم المكي الشريف</Text>
          <Text style={styles.descriptionPortrait}>
            بث مباشر حالي ومُحدث تلقائياً من الحرم المكي الشريف
          </Text>
        </View>
      )} */}
    </View>
